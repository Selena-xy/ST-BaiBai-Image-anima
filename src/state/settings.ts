...
export interface NaiSettings extends BackendConn {
   ...
   * 自动 tag 请求书写哪种提示词规范。
   *
   * - 'nai'（默认）：用 NAI 规范（NAI 面板里的「NAI 规范」），产出 Base Prompt +
   *   原生 Character Prompts（characters[] 数组）。
   * - 'comfy'：用 ComfyUI 规范，产出单串 tag + 一段 nl，**不产 characters[]**。
   *
   * 为什么需要这个开关：第三方接入点（如 latent.moe 这类转发站）后端实际是 ComfyUI
   * 工作流，各角色的 characters[] 到了那边只会被拼成一条逗号串——NAI 的 Character
   * Prompts 结构（含 centers 定位）整块丢失，分角色外貌/动作的绑定信息反而被稀释。
   * 换成 ComfyUI 规范后，发出去的 input 是干净的「tag + nl」两段。
   *
   * ⚠ 只影响**提示词文本怎么写**，不影响出图请求的协议：走的仍是 nai.ts 的
   * /ai/generate-image。角色固定外貌库照常由 changes 维护，与 characters[] 无关。
   */
  promptFormat: 'nai' | 'comfy';
}
...
function naiDefaults(): NaiSettings {
  return {
    ...
    // 默认仍是 NAI 规范:官方站与真 NAI 兼容站吃 characters[] 结构,行为不变;
    // 跑 ComfyUI 转发站(anima 类)的用户在 NAI 面板手动切到「ComfyUI 规范」。
    promptFormat: 'nai',
  };
}
...
function normalizeNai(raw: unknown, def: NaiSettings): NaiSettings {
  ...
  return {
    ...
    artistPresets,
    activeArtistId,
    // 存量配置没有这个键 → 回落 'nai'，与升级前行为完全一致（不改变任何现有用户出图结果）。
    promptFormat: o.promptFormat === 'comfy' ? 'comfy' : 'nai',
  };
}
