...
import { naiComfyFormatOn } from '@/autoTag/prompt';
      ...
      try {
        ...
        const validate = (raw: string) => {
          ...
          if (
            ...
            // 切成 ComfyUI 规范后不再产 characters[]，规范也不要求建档 nl，
            // 此时仍校验会把一次正常输出误判成失败并白白重试（见 naiComfyFormatOn）。
            !naiComfyFormatOn(settings) &&
