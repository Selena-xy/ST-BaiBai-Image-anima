...
export function backendStatus(): BackendStatus {
  ...
  if (backend === 'nai') {
    ...
    // 支持与否只看模型与提示词规范,与配没配齐无关:第三方要在出图前就知道该不该传 characters。
    // 切成 ComfyUI 规范后 characters[] 不再产出,此时仍报 true 会让卡片显示「多角色提示已生效」、
    // 也让 third-party 白名单误以为可以传 characters —— 而 nai.ts 仍会把它拼进 char_captions，
    // 与「不产 characters」的规范自相矛盾(见 NaiSettings.promptFormat)。
    const supportsCharacters =
      naiSupportsCharacterPrompts(model) && settings.nai.promptFormat !== 'comfy';
