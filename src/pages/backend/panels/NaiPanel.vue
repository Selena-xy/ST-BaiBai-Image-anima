...
/** 「提示词规范」下拉选项(见 NaiSettings.promptFormat)。 */
const PROMPT_FORMAT_OPTIONS: { value: string; label: string }[] = [
  { value: 'nai', label: 'NAI 规范(官方/兼容站)' },
  { value: 'comfy', label: 'ComfyUI 规范(转发站/anima)' },
];
    ...
    <div class="bbi-sections">
      ...
      <Collapsible title="默认参数" :open="false">
        ...
        <div class="bbi-field">
          ...
          <BbiSelect
            class="be-select"
            v-model="settings.nai.promptFormat"
            :options="PROMPT_FORMAT_OPTIONS"
            aria-label="提示词规范"
          />
          <p class="bbi-field-hint">
            自动写 tag 时用哪套规范。NovelAI 官方或真 NAI 兼容站选「NAI 规范」；后端实际是
            ComfyUI 的转发站（如 anima 类模型）选「ComfyUI 规范」——那类站不认 NAI 的
            Character Prompts 结构，各角色描述会被拍平成一条串，按 ComfyUI 口径只写
            tag + 自然语言反而更干净。只改提示词写法，出图请求与画师串等设置都不变。
          </p>
        </div>
